...
def fit_P(x, y):
    """Fit P(d)=alpha+beta*min(d,tau), beta unrestricted, with tau chosen by training SSE."""
...
def main():
    ...
    scores_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_GEP_LOOCV_Scores.csv', index=False)
    ...
    decisions_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_NestedModelDecisions.csv', index=False)
    summary_df = pd.DataFrame(route_summary)
    summary_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_ModelMarginSummary.csv', index=False)
    ...
    if locked is not None:
        ...
        m.to_csv(DATA / 'Qatraneh_R17_SymmetricP_vs_R16_Locked.csv', index=False)
    ...
    tail_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_OuterTail.csv', index=False)
    ...
    legacy_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_OuterTail_Legacy35.csv', index=False)
    ...
    jack_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_Jackknife.csv', index=False)
    jack_sum_df.to_csv(DATA / 'Qatraneh_R17_SymmetricP_Jackknife_Summary.csv', index=False)
    ...
    (DATA / 'Qatraneh_R17_SymmetricP_Verification.json').write_text(json.dumps(verification, indent=2), encoding='utf-8')
