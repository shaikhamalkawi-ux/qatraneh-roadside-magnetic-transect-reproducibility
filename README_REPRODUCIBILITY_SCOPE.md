...
For completeness, the earlier 5-m-inclusive 35-window reconstruction is also retained. It yields 30 complete-class tail-best, 2 inner-winner tail-best, and 3 third-model tail-best cases. The three third-model cases are North Field at 5, 6, and 7 m, with G tail-best in all three.

## Apparent-completeness result
Five Qatraneh profiles have C10 = 1.00: the full observed maximum had already appeared by 10 m. Three of those five nevertheless have D* = 12 m. This shows that the principal result does not depend on the illustrative 90% threshold alone.
...
The Daejeon branch requires the official publisher supplementary XLSX from Lee et al. (2020). That third-party workbook is not redistributed. The package includes the documented source reconciliation and derived summaries, but an independent raw-data replay of the Daejeon branch requires obtaining the publisher supplement.
